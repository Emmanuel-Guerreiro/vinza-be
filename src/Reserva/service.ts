...
class ReservaService {
 public async create(dto: CreateReservaDto) {
  return sequelize.transaction(async (transaction) => {
    const evento = await Evento.findByPk(dto.instanciaEventoId, { transaction, lock: transaction.LOCK.UPDATE });
    if (!evento) throw { message: "Instancia de evento no encontrada", status: 404 };
    ...
    let recorrido = await Recorrido.findByPk(dto.recorridoId, { transaction });

    if (!recorrido) {
      recorrido = await Recorrido.create(
        { idUser: dto.recorridoId }, // Ajusta si necesitas más datos
        { transaction }
      );
    }
    ...
    process.nextTick(() => {
      auditEmitter.emit("create", { entity: "reserva", id: reserva.idReserva });
...
public async update(id: number, dto: CreateReservaDto) {
  return sequelize.transaction(async (transaction) => {
    ...
    process.nextTick(() => {
      auditEmitter.emit("update", { entity: "reserva", id: reserva.idReserva });
  ...
  public async delete(id: number) {
    return sequelize.transaction(async (transaction) => {
      ...
      const evento = await Evento.findByPk(reserva.instanciaEventoId, { transaction, lock: transaction.LOCK.UPDATE });
      if (!evento) throw { message: "Instancia de evento no encontrada", status: 404 };
      ...
      process.nextTick(() => {
        auditEmitter.emit("delete", { entity: "reserva", id: reserva.idReserva });
      });
