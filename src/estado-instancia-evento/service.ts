...
import { CreateEstadoInstanciaEventoDto, UpdateEstadoInstanciaEventoDto } from './types';
...
class EstadoInstanciaEventoService {
  ...
  public async findOne(id: number, transaction?: Transaction) {
    const estadoInstanciaEvento = await EstadoInstanciaEvento.findByPk(id, { transaction });
    if (!estadoInstanciaEvento) throw errors.app.instanciaEvento.estado_not_found;
    return estadoInstanciaEvento;
  }
  ...
  public async update(id: number, dto: UpdateEstadoInstanciaEventoDto) {
    ...
    if (!estadoInstanciaEvento) throw errors.app.instanciaEvento.estado_not_found;
    const updatedEstadoInstanciaEvento = await estadoInstanciaEvento.update(dto, {
      returning: true,
    });
